$.extend({
	queryString : function(str){
		...
		while(i < len){
			temp = arr[i++].split(/=/);
			//编解码url上的%形式的数据
			//与java的tomcat服务器进行get请求数据交互时，发送时需要encodeURI两次，接收时需要decodeURI两次！！！！！！
			//如decodeURI(decodeURI(temp[1]));
			result[temp[0]] = decodeURI(temp[1]);
		}
		return result;
	}
});
$(".project").val($.queryString(location.search).project);
$(".owner").val($.queryString(location.search).owner);
$(".hack").focus(function(){
...
$("input").keyup(function(){
	var $parent = $(this).parent();
	$parent.next().html(/<script.*>/.test($(this).val()) ? $parent.prev().text().replace(/：/, "") + "不可输入非法内容" : "");
});
var $lastTr = $("tr:last");
$(".add").click(function(){
...
$("html").keydown(function(e){
	if(e.keyCode === 116){
		e.preventDefault();
		location.search = "?project=" + $(".project").val() + "&owner=" + $(".owner").val();
	}
});
...
//匹配邮箱
console.log("1@a.com".match(/^([a-zA-z\d])\.?\1@[a-z\d]+\.(?:com|cn|com\.cn|org|edu|net)$/));
